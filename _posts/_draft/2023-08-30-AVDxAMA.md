...
New-AzDiagnosticSetting -Name "AVDInsights" -ResourceId (Get-AzWvdHostPool -Name $HostPoolName -ResourceGroupName $ResourceGroup).Id -WorkspaceId $LogWorkspaceId -Log $log -Verbose

# Enable AVDInsights (Diagnostic Settings) for the AVD Workspace
$log = @()
$log += New-AzDiagnosticSettingLogSettingsObject -Enabled $true -CategoryGroup allLogs 
New-AzDiagnosticSetting -Name "AVDInsights" -ResourceId (Get-AzWvdWorkspace -Name $WorkspaceName -ResourceGroupName $ResourceGroup).Id -WorkspaceId $LogWorkspaceId -Log $log -Verbose
