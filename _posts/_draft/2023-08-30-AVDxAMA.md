...
9. The next steps are simple, as the Azure Monitor Agent must be installed on all session hosts and the session hosts must be associated with the data collection rule. So that the session hosts know what monitoring data they need to send to the log analysis workspace.

First, check that all session hosts have a system-managed identity. If not, you can add it here (**Add system managed identity**) to all session hosts that lack it.
